...
export default async function SupplierStartPage({
  ...
  if (userError || !user) {
    redirect(`/ar/sign-in?next=${encodeURIComponent("/ar/supplier/start")}`);
  }
    ...
    <main className="p-6 space-y-6 max-w-xl">
      ...
      {role === "supplier" && !hasSupplierRow ? (
        <div className="rounded border border-border bg-muted/30 p-3 text-sm">
          حسابك يحمل دور <span className="font-medium">المورد</span> بالفعل، لكن الإعداد غير مكتمل.
          يرجى إكمال البيانات أدناه.
        </div>
      ) : null}
