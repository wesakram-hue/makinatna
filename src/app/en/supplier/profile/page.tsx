  ...
  if (role !== "admin" && !supplier?.id) {
    return (
      <div className="space-y-4">
        <div className="text-sm">Supplier profile not found. Please complete setup first.</div>
        <Button asChild>
          <Link href={`/en/supplier/start?next=${encodeURIComponent(next)}`}>Go to setup</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <Button variant="outline" asChild>
          <Link href="/en/supplier">{t(locale, "supplier.dashboardNav")}</Link>
        </Button>
      </div>
      ...
      {error ? (
        <div className="rounded border border-destructive/50 bg-destructive/10 p-3 text-sm text-destructive">
          {error === "missing_name" ? "Please enter a supplier name." : error}
        </div>
      ) : null}
